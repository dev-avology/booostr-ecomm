(function ($) {
    ...
    $('#deposit_perfomace').on('change',function(){

        var period=$('#deposit_perfomace').val();

        load_deposit_perfomace(period);
    });
    ...
    function loadStaticData() {
        ...
        $.ajax({
            ...
            success: function(response){ 
                ...
                $('#total_expired_subscriptions').html(response.total_tax);
    ...
    function load_deposit_perfomace(period) {
        $('#deposit_performance_loader').show();
        var url=$('#deposit_perfomance').val();
        $.ajax({
            type: 'get',
            url: url+'/'+period,
            ...
            success: function(response){ 
                ...
                if (period != 365) {
                    ...
                    load_perfomace_chart(dates,totals,"depositChart");
                }
                else{
                    $.each(response, function(index, value){
                        var month=value.month;
                    ...
                    load_perfomace_chart(month_year,totals,"depositChart");
                }
    ...
    function load_perfomace_chart(dates,totals,id="myChart") {
