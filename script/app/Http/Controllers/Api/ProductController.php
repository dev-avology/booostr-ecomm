...
class ProductController extends Controller
{
   ...
   public function categoryList(Request $request){
    ...
    {
        $info=Term::query()->where('type','product')->where('status',1)->whereIn('list_type', [0,1])->with('tags','brands','excerpt','description','preview','medias','optionwithcategories','price','prices','seo','formFields')->withCount('reviews')->where('id', $id)->first();
        if(empty($info)){
