...
class ProductController extends Controller
{
   ...
   public function categoryList(Request $request){
    ...
    public function productList(Request $request)
    {
       ...
       ->whereIn('list_type', [0,1])->with('media','category','firstprice', 'lastprice','formType')->whereHas('firstprice')->whereHas('lastprice')->selectRaw('*, (SELECT MAX(price) FROM prices WHERE term_id = terms.id) AS max_price, (SELECT MIN(price) FROM prices WHERE term_id = terms.id) AS min_price');
    ...
    {
        $info=Term::query()->where('type','product')->where('status',1)->whereIn('list_type', [0,1])->with('tags','brands','excerpt','description','preview','medias','optionwithcategories','price','prices','seo','formType')->withCount('reviews')->where('id', $id)->first();
        if(empty($info)){
