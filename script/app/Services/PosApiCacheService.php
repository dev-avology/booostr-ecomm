...
class PosApiCacheService
{
    public const TTL = 600;

    private const VERSION_KEY = 'pos_api_cache_version';
    ...
    {
        return 'pos_api:' . $this->tenantId() . ':' . self::VERSION_KEY;
    }
    ...
    {
        return 'pos_api:' . $this->tenantId() . ':v' . $this->version() . ':' . $suffix;
    }
    ...
    public function get(string $suffix): ?array
    {
        try {
            ...
            Log::warning('POS API cache read failed', [
                'suffix' => $suffix,
                'error' => $e->getMessage(),
            ]);
    ...
    public function set(string $suffix, $data, int $ttl = self::TTL): void
    {
        try {
            ...
            Log::warning('POS API cache write failed', [
                'suffix' => $suffix,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    /**
     * Invalidate all POS API read caches for the current tenant (version bump).
     */
    public function bump(): void
    {
        try {
            ...
            Log::warning('POS API cache bust failed', [
    ...
    private function purgeLegacyUnversionedKeys(): void
    {
        try {
            ...
            Log::warning('POS API legacy cache purge failed', [
                'tenant' => $this->tenantId(),
                'error' => $e->getMessage(),
            ]);
        }
