    ...
    $.each(topratedproducts,function(key ,product){
            ...
            var html = `<div class="p-slide-active">
                     <div class="single-p-food align-items-center">
                        <div class="food-text">
                           <h4 class="s-food-title"><a href="${base_url+'/product/'+product.slug}">${escapeHtml(str_limit(product.title,18))}</a></h4>
                           <p class="s-food-price"><span>Starting from</span>${price}</p>
                        </div>
                        <div class="s-food-image">
                           <img src="${preloader}" data-src="${preview}" class="lazy" alt="${escapeHtml(product.title)}">
                        </div>
                     </div>
                  </div>`;
    ...
    $.each(data,function(key ,row){
            ...
            var html = key == 0 ? `
                        <div class="single-recommendation">
                           <div class="image">
                              <a href="${row.link}"><img class="lazy" src="${preloader}" data-src="${preview}" alt="${escapeHtml(row.title)}"></a>
                             
                           </div>
                           <div class="content">
                              <h2><a href="${row.link}">${escapeHtml(row.title)}</a></h2>
                              <ul>
                                 <li><i class="icofont-calendar"></i> ${escapeHtml(row.days)}</li>
                                 <li><i class="icofont-clock-time"></i> ${escapeHtml(row.time)}</li>
                              </ul>
                              <div class="button">
    ...
    success: function(response) {
        ...
        $.each(response.productmenu, function(key, category) {
            var li = `<li class="nav-item" role="presentation">
                        <button class="nav-link filteredtab filter_products filter_product${category.id}" data-id="${category.id}" data-items="[]"  data-bs-toggle="tab" data-bs-target="#filteredtab" type="button" role="tab" aria-controls="2ndtab" aria-selected="true"><img src="${category.icon != null ? category.icon.content : ''}" alt=""> ${escapeHtml(category.name)}</button>
                     </li>`;
            $('.menutab').append(li);
        });
        ...
        $.each(response.shortbanner, function(key, row) {
            ...
            var html=`<div class="col-lg-12 col-md-6 col-12">
                        <div class="small-content ${key == 1 ? 'secound dark' : 'first'} " style=" background-image: url('${row.preview}');">
                           <div class="inner">
                              <h4 class="title">${escapeHtml(row.title)}</h4>
                              ${button}
            ...
            var html=`<div class="container">
                        <div class="row">
                           <div class="col-lg-6 col-md-8 col-12">
                              <h2 class="call-action-title"> ${escapeHtml(row.title)}</h2>
                              <p class="call-action-des">${escapeHtml(row.description)}</p>
                               ${button}
                           </div>
                        </div>
                     </div>`;         
        ...
        $.each(response.latestblogs,function(key, row) {
            ...
            var html=`<div class="col-lg-3 col-md-6 col-12">
                  <div class="shop-single-blog">
                     <div class="image"><a href="${row.url}"><img src="${preloader}" class="lazy" data-src="${row.preview}" alt="${escapeHtml(row.title)}"></a></div>
                     <div class="content">
                        <p class="date"><i class="icofont-calendar"></i>${escapeHtml(row.time)}</p>
                        <h4 class="title"><a href="${row.url}">${escapeHtml(row.title)}</a></h4>
                        <p class="text">${escapeHtml(row.short_description)}</p>
                        <div class="button">
...
function heroSlider(data) {
    ...
    $.each(data, function(key, row) {
        var html=`<div class="big-content" style="background-image: url(${row.preview});">
                           <div class="inner">
                              <h4 class="title">${escapeHtml(row.title)}</h4>
                              <p class="des">${escapeHtml(row.description)}</p>
                              <div class="button">
                                 <a href="${row.link}" class="btn second-btn">${escapeHtml(row.button_text)}</a>
                              </div>
                           </div>
                        </div>`;
