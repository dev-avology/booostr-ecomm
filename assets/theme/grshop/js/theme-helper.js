...
function render_primary_product(products, target,additional_class='',visible_badge=false) {
    ...
    $.each(products, function(key, product) {
        ...
        if (visible_badge == true) {
            ...
            $.each(product.features, function(k, feature) {
                badge =`<div class="p-badge popular">${escapeHtml(feature.name)}</div>`;
    ...
    <div class="product-content">
        ...
        <!-- Product Title -->
        <h3 class="product-title"><a href="${base_url+'/product/'+product.slug}">${escapeHtml(product.title)}</a></h3>
        <p>${product.excerpt != null ? escapeHtml(product.excerpt.value) : ''}</p>
        <!-- Product Price -->
        ...
        ${cart_html}
            ...
            var html=`<div class="single-product ${additional_class}">
                     ...
                     <div class="product-content">
                        ...
                        <!-- Product Title -->
                        <h3 class="product-title"><a href="${base_url+'/product/'+product.slug}">${escapeHtml(str_limit(product.title,20))} </a></h3>
                        <!-- Product Price -->
                        <div class="product-price">
                           <b>${price}</b>
                        </div>
                           ...
                           <div class="row align-items-center">
                              ...
                              <div class="col-lg-7 col-md-7 col-12">
                                 <div class="content">
                                    ...
                                    <p>
                                       ${str_limit(product.excerpt != null ? escapeHtml(product.excerpt.value) : '',30)}
                                    </p>
                                    <div class="price">
                                       <h5>${price}</h5>
                                    </div>
         ...
         var cart_options='';
              $.each(item.options.options, function (option_key, option) 
              {
                ...
                $.each(option, function (child_option_key, child_option_value) 
                 {
                  child_options +=`${escapeHtml(child_option_value.name)},`;
                 })
        ...
        var cart_item=` <li class="cart_item cart_item${item.rowId}">
               <div class="cart-single-main">
                  ...
                  <div class="cart-single-item">
                     <h4><a href="${base_url+'/product/'+item.options.slug}">${escapeHtml(str_limit(item.name,20))}</a><span>${cart_options}</span>
                     </h4>
                            ...
                            <td class="image" data-title="Product">
                                <div class="primary-head-d">
                                    ...
                                    <div class="image-cart-right">
                                        <p class="product-name"><a href="${base_url+'/product/'+item.options.slug}">${escapeHtml(str_limit(item.name,100))}</a></p>
      ...
      success: function(response){ 
        ...
        $.each(response.galleries, function (key, item){
            var html=`<div class="quick_single_slider single-slider">
                           <img src="${item}" alt="${escapeHtml(response.data.title)}">
                        </div>`;
...
function render_price_variation(prices) {
    ...
    <label for="modal_variation${ price.id }">${ escapeHtml(price.category.name) }</label>`;
        ...
        name="option[${row.id}][]" 
        value="${ price.id }">
        <label for=" modal_variation${ price.id }">${ escapeHtml(price.category.name) }</label></div>`;
